...
async function makeBrandProfile(name) {
  ...
  return {
    ...
    history,
    style: [
      ["Colors", personality.palette],
      ["Font Style", uniqueText(`${personality.visual}; adjust the letter spacing around the ${traits.startsWith.toUpperCase()} opening and ${traits.endsWith.toUpperCase()} ending so the wordmark feels custom.`, traits)],
      ["Logo Direction", uniqueText(personality.logo, traits)],
      ["Website Vibe", uniqueText(`${personality.energy} with a layout that emphasizes ${traits.topFactor.label.toLowerCase()} and compensates for ${traits.lowFactor.label.toLowerCase()}.`, traits)]
    ]
  };
}
...
function createNameReveal(name) {
  ...
  [...name].forEach((character, index) => {
    ...
    const x = direction === 0 ? "-46vw" : direction === 1 ? "42vw" : `${Math.round((Math.random() - 0.5) * 80)}vw`;
    const y = direction === 2 ? "-36vh" : direction === 3 ? "36vh" : `${Math.round((Math.random() - 0.5) * 58)}vh`;
    const z = direction === 4 ? "360px" : `${Math.round(Math.random() * 180)}px`;
    letter.style.setProperty("--reveal-x", x);
    letter.style.setProperty("--reveal-y", y);
    letter.style.setProperty("--reveal-z", z);
    letter.style.setProperty("--reveal-rotate", `${Math.round((Math.random() - 0.5) * 90)}deg`);
  ...
  for (let index = 0; index < 34; index += 1) {
    ...
    particle.style.setProperty("--particle-x", `${Math.round(Math.random() * 100)}%`);
    particle.style.setProperty("--particle-y", `${Math.round(Math.random() * 100)}%`);
    particle.style.setProperty("--particle-delay", `${Math.round(Math.random() * 900)}ms`);
...
function showBrandProfile(profile) {
  ...
  styleList.innerHTML = "";
  profile.style.forEach(([label, text]) => {
    const item = document.createElement("div");
    const labelElement = document.createElement("strong");
    const textElement = document.createElement("p");
    labelElement.textContent = label;
